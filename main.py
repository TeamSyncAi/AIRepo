...
def generate_project_modules():
    ...
    for line in generated_content_text.split('\n'):
        if line.startswith("**"):
            ...
            tasks.append({"task_name": task_name, "projectID": project_id})  # Add projectID to the task

    if current_module:
        modules.append({"module_name": current_module, "tasks": tasks})

    print("Generated Modules:", modules)  # Add this line to print the generated modules

    try:
        response = requests.post(NODE_SERVER_URL, json={"projectID": project_id, "modules": modules})
        response_data = response.json()
        print("Response from Node Server:", response_data)  # Add this line to print the response from the Node server
        return jsonify({"message": "Modules sent successfully", "response": response_data})
    except Exception as e:
        print("Error:", str(e))  # Add this line to print any exceptions
